# Script to generate a multiplot of the impact of burstiness (ASPID case with MAP I=1K and Pareto H=0.6)
...
set tics scale 1.5
set output 'burstiness-aspid.eps'
set offsets
set autoscale fix
set size ratio 0.9
...
set style line 3 @LINEWIDTH lt 1 lc rgb "#E2601F"
...
set title 'Elasticity + PID' font "Helvetica,12" offset 0,-0.5,0
...
set y2range[0:35]
set y2label "Thread level and splitting" font "Helvetica,11" offset -4.5,0,0
set y2tics (0, 7, 14, 20) nomirror font "Helvetica,10" offset -1,0,0
plot "results_aspid_i1K/controller.log" using ($1*2.5):2 with linespoints ls 1 title "utiliz. factor", 0.9 with lines ls 2 notitle, "results_aspid_i1K/controller.log" using ($1*2.5):5 with boxes ls 1 axes x1y2 title "thread level", "results_aspid_i1K/controller.log" using ($1*2.5):3 with boxes ls 3 axes x1y2 title "splitting"
...
set y2label "Thread level and splitting" font "Helvetica,11" offset -4.5,0,0
set y2tics (0, 7, 14, 20) nomirror font "Helvetica,10" offset -1,0,0
plot "results_aspid_h0.6/controller.log" using ($1*2.5):2 with linespoints ls 1 title "utiliz. factor", 0.9 with lines ls 2 notitle, "results_aspid_h0.6/controller.log" using ($1*2.5):5 with boxes ls 1 axes x1y2 title "thread level", "results_aspid_h06/controller.log" using ($1*2.5):3 with boxes ls 3 axes x1y2 title "splitting"
unset multiplot
